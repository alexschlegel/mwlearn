...
% 
% Syntax:	h = GO.Plot.Bar(x,err,p,<options>)
%
% In:
...
%		shownsig:	(<auto>) true to indicate non-signficant values
%		dimnsig:	(<auto>) true dim non-significant values
...
%get the plot order
	if ~isempty(opt.groups)
		kGroupOrder	= GO.Plot.Order(opt.groups);
...
%translate
	opt.groups	= GO.Translate(opt.groups);
	opt.bars	= GO.Translate(opt.bars);
