...
function [rc, datafile] =OS(subject,screenparms,yesno,OSParams)
global ptb3
...
%     
    ...
    listlength = OSParams.listLength;  %array of list lengths, each replicated Ntpercond times\
    maxListLength = max(listlength);
    Ntpercond = OSParams.trialsPerCond;       %trials per condition (ie list length)
    ...
    datafile =    PathUnsplit(DirAppend(strDirCode, 'WMBattery','Data'),'OS','dat');
