...
function screenparms = prepexp(mwlt)
...
% Get the screen from mwlt.   
[screenparms.window, ~, screenparms.rect] = mwlt.Experiment.Window.Get('main');
...
switch computer
    ...
    case 'PCWIN'
        ...
        screenparms.displayFont = 'Impact';
    otherwise
        screenparms.serifFont = 'URW Bookman L';
        screenparms.sansSerifFont = 'Arial';
        screenparms.symbolFont = 'Symbol';
        screenparms.displayFont = 'Impact';
end
...
if ~exist(PathUnsplit(pwd,'WMCBattery','m'),'file')
   prepend='../';
else
   prepend=''; 
end
