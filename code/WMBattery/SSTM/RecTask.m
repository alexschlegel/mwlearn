...
function [strDataPath, strSumDataPath] = RecTask(SubID,FileID,IntDate,NumT,RecExp,RecTrial,ShiftAns,Score,Order,RecRT,NumIndex)
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
%  This is the subfunction of SSTM used to record the task data.
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
global strDirCode;
strDirWMData = DirAppend(strDirCode, 'WMBattery', 'Data');
...
strDataPath = PathUnsplit(DirAppend(strDirWMData,'SSTMDetailed'),['Sub-' FileID], 'txt');
DataFile=fopen(strDataPath,'w');

strSumDataPath = PathUnsplit(strDirWMData, 'SSTM','dat');
SumDataFile=fopen(strSumDataPath,'w');
fprintf(SumDataFile,'%s %s %s %s %d\n','SubID','Score','Date','Time',(NumT.Test/5)*40);
...
fprintf(DataFile,'%s %s\n',IntDate,'Spatial Short-Trem Memory Task data');
fprintf(DataFile,'%s %s %s %s %s\n','ID','Trial'...
    ,'Score','RT','NumDot');
for i=1:NumT.Test
    fprintf(DataFile,'%3d %2d %2d %5.3f %d\n',SubID,i,...
...
fprintf(SumDataFile,'%2d %2d %s\n',SubID,sum(Score),IntDate);
