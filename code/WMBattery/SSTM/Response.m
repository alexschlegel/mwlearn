...
% Subfunction of SSTM
% This function used to create a response interface, and record the response
% location data.
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
global ptb3
...
if (rect_screen(3)/rect_screen(4))>4/3%Check if widescreen is used
    ...
    LeftOriginal=151*AdjV+hOffset;
    Board.Left=151*AdjV;
    Board.Right=Board.Left+500;
    p=1;%for widescreen
    for i=1:11
        if ptb3
...
while ~stop
   ...
   % [clicks,MPos(1,1) MPos(1,2),whichButton]=GetClicks(WindowPtr,0)
    ...
    if ((MPos(1,1)>Board.Left*AdjH) && (Board.Right*AdjH>MPos(1,1))) && ((MPos(1,2)>Board.Top*AdjV) && (Board.Bottom*AdjV>MPos(1,2)))
        ...
        if min(min(RecAns))~=0
            if ptb3
                Screen('FillRect',WindowPtr,Text.Color1,[hOffset+400*AdjH-50 (587-20)*AdjV hOffset+400*AdjH+50 (587+5)*AdjV]);
                Screen('FillRect',WindowPtr,Text.Color2,[hOffset+400*AdjH-50+1 (587-20)*AdjV+1 hOffset+400*AdjH+50-1 (587+5)*AdjV-1]);
                Screen('Flip', WindowPtr,[],1);                
            else
                Screen(WindowPtr,'FillRect',Text.Color1,[hOffset+400*AdjH-50 (587-20)*AdjV hOffset+400*AdjH+50 (587+5)*AdjV]);
                Screen(WindowPtr,'FillRect',Text.Color2,[hOffset+400*AdjH-50+1 (587-20)*AdjV+1 hOffset+400*AdjH+50-1 (587+5)*AdjV-1]);
            end
